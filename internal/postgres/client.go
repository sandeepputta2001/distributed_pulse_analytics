...
func (c *Client) ListAlertRules(ctx context.Context, appID string) ([]*models.AlertRule, error) {
	const q = `
		SELECT id, app_id, name, metric_name, condition, threshold,
		       window_mins, channels, webhook_url, email_to, active, created_at
		FROM alert_rules
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.AppID, &r.Name, &r.MetricName,
			...
			&r.WebhookURL, &r.EmailTo, &r.Active, &r.CreatedAt); err != nil {
			return nil, err
		}
...
func (c *Client) ListCohorts(ctx context.Context, appID string) ([]*models.CohortDefinition, error) {
	const q = `SELECT id, app_id, name, description, filter_sql, user_count, created_at FROM cohort_definitions WHERE app_id = $1 ORDER BY created_at DESC`
	rows, err := c.read().Query(ctx, q, appID)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&co.ID, &co.AppID, &co.Name, &co.Description, &co.FilterSQL, &co.UserCount, &co.CreatedAt); err != nil {
			return nil, err
		}
